/**
 * C6 Bank Payment Adapter
 * 
 * Digital bank with full developer portal.
 * Boleto, PIX, saldo, extrato, transfers.
 * 
 * Auth: OAuth2
 * Docs: developers.c6bank.com.br
 */
...
export class C6BankAdapter extends PaymentAdapter {
    readonly provider: PaymentProvider = 'c6bank';
    ...
    protected override async request<T>(
        ...
        if (!response.ok) {
            ...
            throw new PaymentAdapterError('c6bank', `${method} ${path} → ${response.status}: ${errorBody}`, response.status, errorBody);
        }
    ...
    private async ensureAccessToken(): Promise<string> {
        ...
        if (!response.ok) throw new PaymentAdapterError('c6bank', `OAuth failed: ${response.status}`);
    ...
    private async createPixCharge(params: CreateChargeParams): Promise<ChargeResult> {
        ...
        return {
            externalId: cob.txid,
            status: normalizeStatus('c6bank', cob.status || 'ATIVA'),
            provider: 'c6bank',
    ...
    async getCharge(externalId: string): Promise<ChargeStatus> {
        try {
            ...
            return {
                externalId: cob.txid,
                status: normalizeStatus('c6bank', cob.status),
    ...
    async refundCharge(_id: string, _amount?: number): Promise<RefundResult> {
        throw new PaymentAdapterError('c6bank', 'Use PIX devolução for refunds');
    }

    // ─── Subscriptions/Recipients ──────────────────────────

    async createSubscription(_p: CreateSubscriptionParams): Promise<SubscriptionResult> { throw new PaymentAdapterError('c6bank', 'Not supported'); }
    async cancelSubscription(_id: string): Promise<void> { throw new PaymentAdapterError('c6bank', 'Not supported'); }
    async getSubscription(_id: string): Promise<SubscriptionStatus> { throw new PaymentAdapterError('c6bank', 'Not supported'); }
    async createRecipient(_p: CreateRecipientParams): Promise<RecipientResult> { throw new PaymentAdapterError('c6bank', 'Not supported'); }
    ...
    async createTransfer(params: CreateTransferParams): Promise<TransferResult> {
        ...
        throw new PaymentAdapterError('c6bank', 'Only PIX transfers via API');
    }
